...
EMERGENCY_MODE_EXAMPLE = """
Example input: Generate hunav agents data for a simulation where Depict an emergency evacuation where everyone in every rooms run out of their room, to the hallways, then toward the exit in the main hallways. Generate data base on this world data as below: 
{
    "zones": [
...
EMERGENCY_MODE = f"""
You have to pay extra attention to the spawn positions of the agents, make sure all agents are spawned in the navigable regions, and keep at least 0.5 meter away from the walls and other entities.
You also have to pay extra attetion to the groups velocity fields goals points guidance. These goals points are used for a planner to find a path, then this path is used to form a velocity field to guide the groups of agents to go along this path to get from their room to the exits.
You should spawn about 20 to 40 agents depend on the user prompt to depict a chaos situation. The agents should be spawned in rooms, every agents in one rooms belong to one group and use the same velocity field.
{EMERGENCY_MODE_EXAMPLE}
"""
...
NORMAL_MODE = f"""
You have to pay extra attention to the spawn positions and the center of the groups formation, make sure the spawn positions and the center of the groups formation are away from the obstacles, other entities, and consistent with the user prompt.
{NORMAL_MODE_EXAMPLE}
"""

QUEUING_MODE = f"""
You have to pay extra attention to the DIRECTION of the queues and the distance between agents in the queues, make sure the queues are formed in the correct direction and the distances between agents are consistent with the user prompt. 
{QUEUING_MODE_EXAMPLE}
"""
