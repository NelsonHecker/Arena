...
class GeminiEmbeddingFunction(EmbeddingFunction):
    ...
    def __call__(self, input_: Documents) -> Embeddings:
        ...
        response = self.client.models.embed_content(
            model=self.model, contents=list(docs), config=self.config
        )
...
def create_chroma_db(
    ...
    embedding_config: genai.types.EmbedContentConfig = EMBEDDING_CFG,
) -> chromadb.Collection:
    ...
    collection = chroma_client.create_collection(
        ...
        embedding_function=GeminiEmbeddingFunction(
            client=client, model=embedding_model, config=embedding_config
        ),
    )
    ...
    for i, d in tqdm(
        enumerate(documents),
        total=len(documents),
        desc=f"Creating Chroma Collection at {db_path}",
    ):
...
def get_relevant_bt_nodes(
    query: str, collection: chromadb.Collection, n_results=10
) -> str:
...
if __name__ == "__main__":
    ...
    inference_client = genai.Client(api_key=os.environ["GEMINI_API_KEY"])
