...
class GoToPhase(TaskPhase):
    ...
    def is_satisfied(self, robot_manager: "RobotManager") -> bool:
        ...
        tol_dist = (
            ...
            if self.tolerance_radius is not None
            else robot_manager._goal_tolerance_distance  # noqa: SLF001
        )
        tol_ang = (
            self.tolerance_angle
            if self.tolerance_angle is not None
            else robot_manager._goal_tolerance_angle  # noqa: SLF001
        )
...
class TaskRequest:
    ...
    done_predicate: Optional[DonePredicate] = None

    @property
    def kind(self) -> Optional[TaskKind]:
