...
def _build_genai_shim():
    ...
    class _LocalCache:
        def __init__(self, name, system_instruction, contents, model):
            self.name = name
    ...
    class _LocalModels:
        ...
        def generate_content(self, *, model, contents, config=None, **kw):
            ...
            return self._real.generate_content(
    ...
    class _LocalAsyncModels:
        ...
        async def generate_content(self, *, model, contents, config=None, **kw):
            contents, config = _expand(contents, config)
            return await self._real.generate_content(
                model=model, contents=contents, config=config, **kw
            )
...
def _build_openai_shim():
    ...
    class _OpenAIShim:
        ...
        AsyncOpenAI: type[_openai.AsyncOpenAI] = (
            _LocalAsyncOpenAI if _ENDPOINT else _openai.AsyncOpenAI
        )
