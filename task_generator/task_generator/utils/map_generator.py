...
def generate_room_walls(rooms: list[dict]) -> list[FlowList]:
    ...
    for room in rooms:
        ...
        if room["side"] == "top":
            if door_start - x > 0.1:
                walls.append(FlowList([FlowList([x, y]), FlowList([door_start, y])]))
            if (x + w) - door_end > 0.1:
                walls.append(FlowList([FlowList([door_end, y]), FlowList([x + w, y])]))
            walls.append(FlowList([FlowList([x + w, y]), FlowList([x + w, y + h])]))
            walls.append(FlowList([FlowList([x + w, y + h]), FlowList([x, y + h])]))
