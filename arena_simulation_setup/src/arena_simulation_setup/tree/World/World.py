...
class MultiLevelWorld:
    ...
    def render_whole(
        ...
        return flattened_world.render(resolution)[0]
    ...
    def render_individually(
        ...
        return ((floor_id, floor.render(resolution)) for floor_id, floor in self.levels.items())
    ...
    def export(
        ...
        files['map/map.png'] = self.render_whole(resolution=resolution)
        whole_origin = (5 * resolution, 5 * resolution)
        ...
        for floor_id, (png, origin) in self.render_individually(resolution=resolution):
...
class MultiLevelWorldView(PathView):
    ...
    # TODO : revise
    def save(self, multi_level_world: MultiLevelWorld, validate: bool = True) -> Path:
        if validate:
            multi_level_world.validate()

        os.makedirs(self.path, exist_ok=True)
        with open(self.world_path, 'w', encoding='utf-8') as f:
            yaml.safe_dump(
                converter.unstructure(multi_level_world),
                f,
                sort_keys=False,
            )
        return self.path
