...
def _run_report(args: dict, bridge: EvalBridge | None = None) -> dict:
    benchmark_id = args["benchmark_id"]
    cmd_args = ["report", "--benchmark-dir", benchmark_id]
    manifest = args.get("report_manifest", "")
    if manifest:
        if bridge is not None and "/" not in manifest and "\\" not in manifest:
            stem = manifest[:-5] if manifest.endswith(".yaml") else manifest
            cand = bridge.benchmark_dir(benchmark_id) / f"{stem}.yaml"
            if cand.is_file():
                manifest = str(cand)
        cmd_args.extend(["--report-manifest", manifest])
...
def _read_benchmark_status(args: dict, bridge: EvalBridge) -> dict:
    ...
    all_done = all(s in ("ok", "failed", "partial", "skipped") for s in statuses)

    return {
        "run_id": (manifest or {}).get("run_id", bid),
        "suite": (manifest or {}).get("suite_name", ""),
        "contest": (manifest or {}).get("contest_name", ""),
        "status": "completed" if (all_done and statuses) else (
            "in_progress" if statuses else "unknown"
        ),
...
def _compare_planners(args: dict, bridge: EvalBridge) -> dict:
    ...
    return {
        "benchmark_id": bid, "planner_column": planner_col,
...
def _compute_correlation(args: dict, bridge: EvalBridge) -> dict:
    ...
    metrics = [m for m in args.get("metrics", []) if m in df.columns]
    if len(metrics) < 2:
    ...
    return {"benchmark_id": bid, "metrics": metrics, "correlation_matrix": matrix}
...
def _find_top_n(args: dict, bridge: EvalBridge) -> dict:
    ...
    n = int(args.get("n", 3))
    metrics = [m for m in (args.get("metrics") or _default_compare_metrics(bridge))
               if m in df.columns]
    weights = args.get("weights") or [1.0] * len(metrics)
    declarations = _metric_declarations(bridge)
    ...
    return {"benchmark_id": bid, "top_n": ranked[:n]}
...
def _load_notes_file(bid: str, bridge: EvalBridge) -> list[dict]:
    notes_path = bridge.benchmark_dir(bid) / "notes.yaml"
    if not notes_path.exists():
        return []
    try:
        data = yaml.safe_load(notes_path.read_text())
...
def _save_notes_file(bid: str, rows: list[dict], bridge: EvalBridge) -> None:
    notes_path = bridge.benchmark_dir(bid) / "notes.yaml"
    notes_path.parent.mkdir(parents=True, exist_ok=True)
    notes_path.write_text(yaml.safe_dump(rows, sort_keys=False, allow_unicode=True))
...
def _write_notes(args: dict, bridge: EvalBridge) -> dict:
    ...
    if mode == "replace":
        ...
        rows = _load_notes_file(bid, bridge) + list(notes)
    elif mode == "merge":
        existing = _load_notes_file(bid, bridge)
    ...
    _save_notes_file(bid, rows, bridge)
    return {"benchmark_id": bid, "n_notes": len(rows), "mode": mode}
