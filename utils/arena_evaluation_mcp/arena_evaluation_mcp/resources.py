...
def read_resource_content(
    ...
    if uri == "arena_eval://benchmarks":
        ...
        return _json_result(runs)

    if uri == "arena_eval://manifests":
        return _json_result(bridge.discover_available_manifests())

    if uri == "arena_eval://suites":
        return _json_result(bridge.list_suite_stems())

    if uri == "arena_eval://contests":
        return _json_result(bridge.list_contest_stems())

    if uri == "arena_eval://maps":
        return _json_result(bridge.discover_available_maps())

    if uri == "arena_eval://robots":
        return _json_result(bridge.discover_available_robots())
    ...
    if uri.startswith(prefix):
        ...
        if sub == "metrics/schema":
            ...
            if df is None:
                return _json_result({"error": "No combined_metrics.parquet found"})
        ...
        if sub == "notes":
            ...
            return _json_result({"benchmark_id": benchmark_id, "notes": notes})

        if sub == "report":
            report_path = bridge.benchmark_dir(benchmark_id) / "report.html"
            return _text_result(
                str(report_path) if report_path.exists() else "null"
            )

    return _json_result({"error": f"unknown resource: {uri}"})



def _json_result(data: object) -> ReadResourceResult:
...
def _text_result(text: str) -> ReadResourceResult:
