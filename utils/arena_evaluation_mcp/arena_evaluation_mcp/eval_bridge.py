...
class EvalBridge:
    ...
    def list_benchmarks(
        ...
        for run_path in sorted(root.iterdir(), reverse=True):
            ...
            try:
                ...
                run_status = (
                    "completed"
                    if (statuses and all(s in ("ok", "failed", "partial", "skipped") for s in statuses))
                    else ("in_progress" if statuses else "unknown")
                )
    ...
    def discover_available_manifests(self) -> list[str]:
        ...
        try:
            ...
            bundled.update(available_manifests())
        except Exception:
            bundled.update(
                ["standard", "social", "safety", "ecological", "characterization"]
            )
    ...
    def _find_benchmark_runners(self) -> list[tuple[int, str]]:
        """All benchmark runner processes found on the system via ps."""
        out = subprocess.run(
            ["ps", "-eo", "pid=,args="],
        ...
        for line in out.splitlines():
            ...
            if args.startswith(("pkill ", "ps ", "grep ")):
                continue
            if ("evaluation benchmark" in args
                    or "lib/arena_evaluation/benchmark" in args):
                found.append((pid, args[:100]))
        return found

    def stop_benchmark(
        self, run_id: str | None = None, force: bool = False
    ) -> dict:
        """Stop a running benchmark and clean up the simulation environment."""
        pids: set[int] = set()
        if run_id:
            ...
            if proc is None:
                return {"error":
                        ...
                        "started before the server restarted). Omit run_id to "
                        "stop all running benchmarks."}
            if proc.poll() is None:
                pids.add(proc.pid)
        else:
            pids.update(
                p.pid for p in self._bg_processes.values() if p.poll() is None
            )
            pids.update(pid for pid, _ in self._find_benchmark_runners())
        ...
        for sig in sigs:
            ...
            for pid in list(pids):
                try:
                    os.killpg(pid, sig)  # runner is a session/group leader
                    continue
                except (ProcessLookupError, PermissionError):
                    pass
                try:
                    ...
                    pids.discard(pid)
            time.sleep(4 if sig != signal.SIGKILL else 2)
            pids = {p for p in pids if self._process_alive(p)}

        leftover = sorted(str(p) for p in pids)
        cleaned = self._cleanup_orphaned_sim(force)
        return {
            "stopped": leftover or "all stopped",
            "sim_orphans_cleaned": cleaned,
            "note": "run_id omitted → stopped every running benchmark runner",
        } if run_id is None else {
            "run_id": run_id,
            "stopped": leftover or "stopped",
            "sim_orphans_cleaned": cleaned,
        }

    def _cleanup_orphaned_sim(self, force: bool = False) -> list[str]:
        """Kill leftover sim processes so a new benchmark can start.

        pkill patterns use the bracket trick so they never match the
        ...
        first = "KILL" if force else "TERM"
        for p in patterns:
            subprocess.run(
                ["pkill", f"-{first}", "-f", p], capture_output=True, timeout=15
            )
        if not force:
            time.sleep(3)
            for p in patterns:
                subprocess.run(
                    ["pkill", "-KILL", "-f", p], capture_output=True, timeout=15
                )
        # What's still alive
        out = subprocess.run(
            ["ps", "-eo", "args="], capture_output=True, text=True, timeout=15
        ).stdout
        remaining = [
            line.strip()
            for line in out.splitlines()
            for p in patterns
            if re.search(p.replace("[", r"\[").replace("]", r"\]"), line)
            and not line.startswith(("pkill ", "ps ", "grep "))
        ]
        return remaining
