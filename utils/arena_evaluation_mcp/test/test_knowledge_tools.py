...
class TestConfigTemplates:
    ...
    def test_list_suites_and_contests(self, bridge):
        assert len(bridge.list_suite_stems()) >= 3
        assert len(bridge.list_contest_stems()) >= 3
...
class TestDescribeMetric:
    def test_known_metric(self, bridge):
    ...
    def test_unknown_metric(self, bridge):
        from arena_evaluation_mcp.tools import _describe_metric
    ...
    def test_directions_declared_by_metric(self, bridge):
    ...
    def test_default_compare_metrics_from_declarations(self, bridge):
...
class TestPlannerCatalog:
    def test_catalog_shapes(self, bridge):
        ...
        assert cat["local_planners"]  # non-empty
